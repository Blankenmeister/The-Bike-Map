...
<div class="titre">Modifier un parcours</div>
...
<form action="<?= Domain . HOME_URL . 'dashboard/route/modify' ?>" method="POST" class="mb-4" onsubmit="return validateFormCreateRoute();" enctype="multipart/form-data">
  ...
  <div class="mb-3 d-flex flex-row justify-content-between">
    ...
    <input type="text" id="name" name="name" autocomplete="name" class="form-control add-name" required placeholder="Entrez le nom de votre parcours" value="<?= $route->getName(); ?>" >
  </div>

  <div class="mb-3 d-flex flex-row justify-content-between">
    <label for="description" class="form-label">Description :</label>
    <textarea type="textarea" id="description" name="description" autocomplete="description" class="form-control add-description" required placeholder="Entrez le texte descriptif" rows="5" ><?= $route->getDescription(); ?></textarea>
  </div>

  <div class="mb-3 d-flex flex-row justify-content-between">
    <label for="duration" class="form-label">Durée :</label>
    <input type="time" id="duration" name="duration" autocomplete="duration" class="form-control add-duration" required placeholder="Entrez la durée du parcours" value="<?= $route->getDuration(); ?>">
  </div>

  <div class="mb-3 d-flex flex-row justify-content-between">
    <label for="distance" class="form-label">Distance :</label>
    <input type="number" id="distance" name="distance" autocomplete="distance" class="form-control add-distance" required placeholder="Entrez la distance du parcours (en km)" value="<?= $route->getDistance(); ?>">
  </div>

  <div class="mb-3 d-flex flex-row justify-content-between">
    <label for="elevation" class="form-label">Dénivelé + :</label>
    <input type="number" id="elevation" name="elevation" autocomplete="elevation" class="form-control add-elevation" required placeholder="Entrez le dénivelé (en m)" value="<?= $route->getElevation(); ?>">
  </div>

  <div class="mb-3 d-flex flex-row justify-content-between">
    <label for="altitude" class="form-label">Altitude :</label>
    <input type="number" id="altitude" name="altitude" autocomplete="altitude" class="form-control add-altitude" required placeholder="Entrez l'altitude (en m)" value="<?= $route->getAltitude(); ?>">
  </div>
